                    ...
                    <table class="w-full text-sm text-left text-gray-500">
                        ...
                        <tbody>
                            ...
                            <tr class="{{$cdata['id'] != $item['id']? 'bg-white hover:bg-gray-50' : 'bg-yellow-100 hover:bg-orange-100'}}" class=" border-b">
                                <td scope="row" class="py-4 px-6 text-center">
                                    @if ($item['image'] != '')
                                ...
                                <td class="py-4 px-6 text-center">
                                    ...
                                    <td class="py-4 px-6 text-right">
                                        <a class="font-medium text-blue-600 hover:underline" href="berita?{{$page != 1? 'page='.$page.'&' : ''}}content={{$item['id']}}">Edit</a>
                ...
                <div class="w-full lg:w-1/2 {{$cdata['id'] == 0? 'bg-white' : 'bg-yellow-100'}}">
                    ...
                    <div class="flex flex-col gap-2 shadow p-3">
                        <div class="flex justify-between">
                            <h3 class="font-bold">{{$cdata['id'] == 0? 'Buat Berita' : 'Edit Berita'}}</h3>
                            @if($cdata['id'] != 0)
                                <a href="{{route('admin.berita')}}" class="px-4 py-2 text-sm font-medium text-white bg-red-600 border border-transparent rounded-md hover:bg-red-500">Batal Edit</a>
                            @endif
                        </div>
                        <hr>
                    
                        <form action="{{$cdata['id'] == 0? route('admin.store') : route('admin.update', ['id' => $cdata['id']])}}" method="POST" enctype="multipart/form-data">
                            @csrf
                            ...
                            <div class="mt-1 rounded-md shadow-sm">
                                <input id="title" name="title" type="text" value="{{$cdata['title']}}" required autofocus class="w-full px-3 py-2 border border-gray-300 rounded-md placeholder-gray-400 focus:outline-none focus:ring-blue focus:border-blue-300 transition duration-150 ease-in-out sm:text-sm sm:leading-5" placeholder="Judul Berita" />
                            </div>

                            <label for="image" class="mt-3 block text-sm font-medium text-gray-700 leading-5">
                                {{$cdata['image'] == ''? 'Image' : 'Change Image'}}
                            </label>
                            <div class="mt-1 rounded-md shadow-sm">
                                <input id="image" name="image" type="file" class="w-full px-3 py-2 border bg-white border-gray-300 rounded-md placeholder-gray-400 focus:outline-none focus:ring-blue focus:border-blue-300 transition duration-150 ease-in-out sm:text-sm sm:leading-5" >
                            </div>
                            ...
                            <div class="mt-1 rounded-md shadow-sm">
                                <textarea id="body" name="body" type="text" required autofocus class="w-full px-3 py-2 border border-gray-300 rounded-md placeholder-gray-400 focus:outline-none focus:ring-blue focus:border-blue-300 transition duration-150 ease-in-out sm:text-sm sm:leading-5" placeholder="Isi Berita" >
                                    {{$cdata['body']}}
                                </textarea>
                            </div>
